...
- 用高频、短小、针对性的练习题，配合实验反馈和 LLM 评测形成学习闭环。
...
4. 学生向 tutor 提问或完成练习后，系统在隐私策略允许的范围内记录 interaction / feedback。
5. 后续把练习题成绩、LLM 评测结果、实验日志、历史对话摘要和做题记录也写入学生模型。

当前 policy engine 是确定性的，故意先做简单可解释版本：未掌握先修时不推荐后续节点，`needs_review` 优先于新节点，掌握度足够高的节点跳过。

后续的 LLM 学习建议不能只看当前问题本身，而要基于更完整的个性化信息：学生 profile、各知识节点掌握度、最近 tutor 对话摘要、做题记录、错因、实验失败日志和反馈记录。也就是说，LLM 负责把“为什么下一步建议做这个”讲清楚，但推荐依据必须来自可审计的学生状态和课程图谱。
...
最终版本希望把“是否记录学生对话历史”做成可手动开启/关闭的课程级或管理员级设置，而不是只靠环境变量。建议保留三层粒度：

- `off`：默认模式，只保存结构化 interaction metadata，不保存原始对话。
- `summary`：保存经过脱敏和压缩的学习摘要，用于个性化建议。
- `full`：保存完整对话，仅用于明确授权的研究、debug 或教师复盘场景。

无论哪种模式，public config 只能暴露当前 logging 是否启用，不能暴露对话内容本身。
...
用户明确希望 OSeduc 有针对性出练习题功能，并且可以由 LLM 负责评测题目。这应该是下一阶段的重点。题目来源需要同时支持教师/助教手动添加和 LLM 自动生成；LLM 生成的题目默认应进入待审核状态，经过人工确认后再面向学生发布。
...
- 是否允许 LLM 评测。
- 是否允许 LLM 生成提示。
- 创建方式：`manual`、`llm_generated` 或 `imported`。
- 审核状态：`draft`、`needs_review`、`published`、`archived`。
...
出题频率可以略高，尤其是每个比较独立的知识模块都应该有短练习。设计上不应等到一章结束才集中测试，而是在学生完成一个独立概念后立刻给 1 到 3 道小题，用动手回答帮助学生暴露误区。推荐节奏可以是：

- 每个 atomic knowledge node 至少 1 道低成本检查题。
- 每个较难节点配置 1 道概念题、1 道简答题、1 道实验诊断题。
- 学生 tutor 对话后，如果还没有做过该节点练习，优先推荐一道短题。
- 学生连续答错时减少新题推进，转为先修节点补救题。
- 学生连续答对时提高题目难度或推荐下一节点。
...
- 教师/助教在后台选择 knowledge node。
- 填写题干、题型、难度、rubric、参考来源和是否允许 LLM 评测。
- 保存为 `draft`，审核后发布。
- 适合核心知识点、考试风格题、课程必须掌握题。

LLM 自动生成路径：

- 教师或系统指定 knowledge node、题型、难度、数量和生成约束。
- 后端取该节点的 source-grounded context、learning objectives、common misconceptions。
- LLM 生成题目、参考答案、rubric、预期误区和 citation。
- 后端校验输出结构、source/citation、难度字段和是否疑似泄露 reference 原文。
- 生成结果默认进入 `needs_review`，由教师/助教人工确认后才发布。

LLM 自动生成题目不能直接把 rCore 原文长段落改写成题干，也不能生成缺少来源的题。生成 prompt 要明确要求“基于来源但用原创表述”，并保留 citation metadata。
...
- `POST /v1/admin/exercises/generate`
- `PUT /v1/admin/exercises/{exercise_id}`
- `POST /v1/admin/exercises/{exercise_id}/publish`
...
admin exercise endpoint 必须复用 admin token 或后续教师权限系统，不能公开给学生端直接调用。LLM 生成题目接口也要限流和记录生成成本。
...
- 如果学生刚完成一个独立知识模块，优先推荐 1 到 3 道短练习巩固。
- 如果学生练习低分，推荐先修节点或更简单题。
- 如果学生连续高分，推荐下一章或更难题。
- 如果学生过去对话中反复提到某个困惑点，把相关 misconception 转成练习推荐依据。
- 如果学生历史做题记录显示某类题型薄弱，下一步建议应包含同知识点的不同表达方式或更基础题。

### 6.8 LLM 个性化下一步建议

后续需要新增一个和 tutor chat 区分开的 “next action explanation” 能力。它可以由 LLM 负责生成自然语言解释，但输入必须来自后端聚合出的学生状态摘要，而不是直接把所有原始历史无筛选塞给模型。

建议输入：

- `StudentProfile`
- 当前 `LearningPath`
- 最近 N 条 `StudentNodeProgress`
- 最近 N 条 tutor interaction metadata 或脱敏摘要
- 最近 N 次 exercise attempt / evaluation
- 相关 knowledge node 和 source citations
...
- 为什么推荐这一步。
- 预计需要多长时间。
- 如果学生觉得太难，应该回退到哪个先修节点。
- citations 和 safety flags。

隐私要求：

- 如果对话历史记录为 `off`，只能使用 interaction metadata，不能使用原始问题。
- 如果为 `summary`，只能使用脱敏摘要。
- 如果为 `full`，也要限制发送给 LLM 的窗口大小和字段，只发送和当前推荐相关的内容。
...
- 练习题页：展示推荐练习、提交答案、查看 LLM 评测反馈。
- 教师题库页：手动添加题目、调用 LLM 生成题目、审核并发布题目。
- 课程设置页：手动开启/关闭学生对话历史记录，选择 `off` / `summary` / `full`。
...
下一阶段应先手写少量 ch4/ch5 练习题 seed，覆盖地址空间、页表、进程、fork/wait。题目必须有 rubric 和 source metadata。随后再做 LLM 自动生成题目，但生成结果必须默认进入待审核状态，不能直接发布给学生。
...
- 更正式的非本队来源说明。
- 运行截图或 smoke 测试记录。
- 练习题手动创建、LLM 生成、人工审核和 LLM 评测流程图。
- 对话历史记录开关的隐私策略说明。
...
4. 先做手动题库：新增 exercise domain、migration、manual create/list/detail API。
5. 手写 ch4/ch5 少量练习题 seed。
6. 增加 exercise attempt API，并用自动评分题跑通 attempt -> evaluation -> progress update。
7. 增加 LLM grading trait 和 mock grading provider。
8. 增加 LLM exercise generation draft API，生成结果默认 `needs_review`。
9. 增加对话历史记录开关的 runtime/course-level 配置设计。
10. 再考虑真实 LLM 评测和 LLM 自动出题联调。
